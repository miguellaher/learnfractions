...
public class MultiplyDivideMixedFractionsSeatWork extends SeatWork {
    ...
    public MultiplyDivideMixedFractionsSeatWork(String topicName) {
        super(topicName);
        String id = AppIDs.MF2S;
        setId(id);
    ...
    public void setQuestions(){
        ...
        for (int i = 0; i < requiredCorrects; i++){
            if (i>=(requiredCorrects/2)) {
                MultiplyingMixedFractionsQuestion question = new MultiplyingMixedFractionsQuestion(range);
                while (questions.contains(question)) {
                    question = new MultiplyingMixedFractionsQuestion(range);
                }
                questions.add(question);
            } else {
                DividingMixedFractionsQuestion question = new DividingMixedFractionsQuestion(range);
                while (questions.contains(question)) {
                    question = new DividingMixedFractionsQuestion(range);
                }
                questions.add(question);
            }
        }
    ...
    public void setGuiFractions(){
        FractionQuestionClass question = questions.get(questionNum);
        if (question instanceof MultiplyingMixedFractionsQuestion){
            MultiplyingMixedFractionsQuestion fractionsQuestion = (MultiplyingMixedFractionsQuestion) question;
        ...
        } else if (question instanceof DividingMixedFractionsQuestion){
            DividingMixedFractionsQuestion fractionsQuestion = (DividingMixedFractionsQuestion) question;
            String tag = fractionsQuestion.getTAG();
            txtSign.setText("÷");
    ...
    public class BtnChoiceListener implements View.OnClickListener{
        ...
        public void onClick(View v) {
            ...
            if (getCurrentItemNum()>getItems_size()){
                ...
                if (TYPE!=null) {
                    if (TYPE.equals(AppConstants.CHAPTER_EXAM)) {
                        AppCache.postSeatWorkStat(MultiplyDivideMixedFractionsSeatWork.this);
